...
class SaleCalculatorTab extends StatefulWidget {
  final String projectType;
  final Map<String, dynamic>? projectData;
  final Function(Map<String, dynamic>)? onProjectSaved;

  const SaleCalculatorTab({
    Key? key,
  ...
  @override
  _SaleCalculatorTabState createState() => _SaleCalculatorTabState();
}

class _SaleCalculatorTabState extends State<SaleCalculatorTab> {
  ...
  final _pricePerSqmController = TextEditingController(); // Цена за кв.м при продаже
  ...
  final double _capRate = 10.0; // Cap Rate 10%
  ...
  void _loadProjectData(Map<String, dynamic> projectData) {
    setState(() {
      ...
      _pricePerSqmController.text = (projectData['pricePerSqm'] ?? '').toString();
    });
  }
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(_isEditMode
            ? 'Редактирование: ${widget.projectData?['name'] ?? 'Продажа'}'
            : 'Калькулятор продажи'),
        centerTitle: true,
        backgroundColor: _isEditMode ? Colors.blue : Colors.blue,
      ),
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                'Расчет окупаемости при продаже недвижимости',
                style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold),
              ),
              const SizedBox(height: 10),
              const Text(
                'Заполните данные для расчета доходности от продажи',
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      // Цена за кв.м при продаже
                      TextFormField(
                        controller: _pricePerSqmController,
                        keyboardType: TextInputType.number,
                        decoration: const InputDecoration(
                          labelText: 'Цена продажи за кв.м (руб) *',
                          border: OutlineInputBorder(),
                          prefixIcon: Icon(Icons.monetization_on),
                        ),
                        validator: (value) {
                          if (value == null || value.isEmpty) {
                            return 'Введите цену за кв.м';
                          }
                          if (double.tryParse(value) == null) {
                            return 'Введите число';
                          }
  ...
  void _saveProject() {
    if (_formKey.currentState!.validate()) {
      ...
      // Рассчитываем общую цену продажи
      double area = double.parse(_areaController.text);
      double pricePerSqm = double.parse(_pricePerSqmController.text);
      double totalSalePrice = area * pricePerSqm;
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            Text('Цена продажи: ${projectData['totalSalePrice']} руб'),
            Text('Цена за кв.м: ${projectData['pricePerSqm']} руб'),
          ],
        ),
        actions: [
          TextButton(
